...
const encrypt = async (
  ...
  const iv = buf2Bigint(mimc7.multiHash(plaintext, BigInt(0))) // Generate the IV
  console.log('iv = ',iv);

  const ciphertext: Ciphertext = {
    iv,
    data: plaintext.map((e: bigint, i: number): bigint => {
      return e + buf2Bigint(mimc7.hash(sharedKey, iv + BigInt(i),)) // Hash the shared key with the IV, then encrypt the plain text
    }),
  }
...
const decrypt = async (
  ...
  const mimc7 = await buildMimc7();
  console.log('ciphertext.iv = ',ciphertext.iv);
